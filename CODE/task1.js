...
//1--Function to take pokemon’s name as argument and display the information of that pokemon

function DisplayPokemon(n)
{
var i,j,flag=0;

for (i in fullGame.pokemon) 
 {

  if(n==fullGame.pokemon[i].name)
   {
      alert("Pokemon ID:"+fullGame.pokemon[i].id);
      alert("Pokemon Num:"+fullGame.pokemon[i].num);
      alert("Pokemon Name:"+fullGame.pokemon[i].name);
      alert("Pokemon Image:"+fullGame.pokemon[i].img);
      alert("Pokemon type:"+fullGame.pokemon[i].type);
      alert("Pokemon height:"+fullGame.pokemon[i].height);
      alert("Pokemon weight:"+fullGame.pokemon[i].weight);
      alert("Pokemon candy:"+fullGame.pokemon[i].candy);
      alert("Pokemon egg:"+fullGame.pokemon[i].egg);
      alert("Pokemon spawn_chance:"+fullGame.pokemon[i].spawn_chance);
      alert("Pokemon multipliers:"+fullGame.pokemon[i].multipliers);
      alert("Pokemon weaknesses:"+fullGame.pokemon[i].weaknesses);
         for (j in fullGame.pokemon[i].prev_evolution) {
          alert("pokemon prev_evolution \n")
         alert("Pokemon num"+fullGame.pokemon[i].prev_evolution[j].num);
        alert("Pokemon Name"+fullGame.pokemon[i].prev_evolution[j].name);
   }

      for (j in fullGame.pokemon[i].next_evolution) 
        {
          alert("pokemon next_evolution \n")
          alert("Pokemon num"+fullGame.pokemon[i].next_evolution[j].num);
          alert("Pokemon Name"+fullGame.pokemon[i].next_evolution[j].name);
        }
         flag=0;
           break;
    ...
    if(flag==1)
    alert("Incorrect Pokemon Name Please Enter Correct PokemonName");
}

alert("1--Finding Pokemon information using pokemon name");
var n=prompt("Enter the Pokemon Name","");
DisplayPokemon(n);


//2--Function that takes pokemon’s name as an argument and finds out which all pokemon have that name in their “next_evolution” field.

function findnextEvolutionPokemon(n)
{
  var i,j,flag=0;
  ...
  { 
     for (j in fullGame.pokemon[i].next_evolution) 
     {
          if(n==fullGame.pokemon[i].next_evolution[j].name)
          {
           alert("Pokemon Name whose next_evolution was is::"+fullGame.pokemon[i].name)
           flag++;
          }
      }
  }
   if(flag==0)
   {
     alert("No Pokemon have given pokemon name as next evolution pokemon")
   }
 }
 
 
alert("2--Find out which all pokemon have given name in their next_evolution"); 
var o=prompt("Enter the Pokemon name to find out which all pokemon have that name in their next_evolution field","");
findnextEvolutionPokemon(o);




//3--Function that take a “Weakness” as an input and gives the names of all pokemon who have that Weakness.

function findWeaknessofPokemon()
{
   var i,j,flag=0;
   var n=prompt("Enter the weakness of pokemon to find which pokemon have that weakness","");
  for (i in fullGame.pokemon)
  { 
     for (j in fullGame.pokemon[i].weaknesses) 
     {
       
          if(n==fullGame.pokemon[i].weaknesses[j])
          {
          alert("Pokemon Name with weakness "+n+" is::"+fullGame.pokemon[i].name)
          flag++;
          }
      }
  }
  if(flag==0)
  {
     alert("No pokemon have given weakness")
  }

}

alert("3--Find the pokemon which have given weakness");
findWeaknessofPokemon();
